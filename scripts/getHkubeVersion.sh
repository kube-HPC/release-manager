...
if [ -z $DEV_VERSION ]; then
  ...
  helm repo add --force-update hkube https://hkube.org/helm
else
  HKUBE_CHART_REPO="hkube-dev/hkube"
  helm repo add --force-update hkube-dev https://hkube.org/helm/dev
fi
helm repo update
...
curl -Lo hkubectl.exe https://github.com/kube-HPC/hkubectl/releases/latest/download/hkubectl-win.exe
curl -Lo hkubectl-macos https://github.com/kube-HPC/hkubectl/releases/latest/download/hkubectl-macos
if [ ! -z $PREV_VERSION ]
then
...
if [ ! -z $COMPRESS ]; then
    ...
    tar cfz ${TAR_FILE_NAME} hkube dockers image-export-import hkubectl*
    echo ${VERSION} is ready at $(realpath ${TAR_FILE_NAME})
fi
